...
value of all other fields in the delta will be zero.
...
A semi-exact day is defined as the same clock time on two successive
days.  So noon to noon is 1 day (even though it may not be exactly 24
hours due to a daylight saving time transition).  A week is defined as
7 days. This is described in more detail below.
...
actually refers to.  Some relationships between fields in the delta
...
$subtract argument is used to specify exactly how the approximate
