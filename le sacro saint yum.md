avant tout par yum installer ces paquets :
+ mlocate (permet de localiser des fichier)
  (locate execute une recherche dans sa base de donné lancer updatedb a l'installation et aprés de grosse modification dans l'arborescence de votre machine)
+ vim (un editeur avec une coloration syntaxique pratique mais pas obligatoire)
+ git (obligatoire pour faire les install)
+ unzip (permet de dezipper les .zip {qui ne sont normalement pas pris en charge par UNIX})

installer tout centreon avec yum
quasiment tout les plugin widget et module (même ceux inutile ou incomplet attention) de centreon peuvent être installer en une seul commande,
en réalisant un yum search centreon
on peut voir tout les paquets disponible pour centreon
!! le paquet de debuginfo pour nrpe3 n'est pas signé il fera planter l'installation des autres paquets !!
pour tout installer taper
yum install -y --exclude=*nrpe3* centreon-*
  -y repond oui automatiquement au question d'installation (pas d'inquiétude il n'y pas de danger a repondre oui a tout)
  --exclude=regexp permet d'exclure de l'installation tout les paquet correspondant a l'expression reguliere
      (ici on exclu nrpe)
  si vous ne voulez pas le detail de l'installation vous pouvez rajoutez -q (quiet) pour activer l'installation silencieuse
  
l'installation par yum permet de gerer automatiquement les probleme de dépendance car yum ira lui-m$eme cherchez les paquets nécéssaire 

Néanmoins prenez garde en installant tout car certain modules ou widget ou autre sont soit inutile imcomplet ou obsolete donc chercher ce dont VOUS avez
besoin pour centreon

un MUST a avoir est :
+ centreon-awie (permet d'importer et d'exporter tout ce que vous avez fais sur centreon web)
  
  
si vous rencontrez des probleme avec yum tel q'une erreur 404 referez vous au fichier note centreon
